...
# Ali Akbar Start (Gap 1 — imports for adaptive approval classification)
from ...approval_guard import ApprovalGuard, BaseApprovalGuard
from ...injection_gateway import scan_and_gate
from ...task_classifier import classify_task, classify_task_with_keywords, POLICY_MAP, get_risk_score
from ...llm_risk_estimator import estimate_risk_with_llm, LLMRiskResult
# Ali Akbar End (Gap 1)
...
class Orchestrator(BaseGroupChatManager):
    ...
    def __init__(
        ...
        # Ali Akbar Start (Gap 1 — accept approval_guard to update policy at task classification time)
        ...
        # Ali Akbar Start (Gap 1 — store approval_guard reference for adaptive policy updates)
        self._approval_guard: BaseApprovalGuard | None = approval_guard
        # Ali Akbar End (Gap 1)
    ...
    async def _orchestrate_step_planning(
        ...
        # Capture the raw TextMessage metadata before overwriting with HumanInputFormat
        _raw_msg = self._state.message_history[-1]
        _raw_metadata = getattr(_raw_msg, "metadata", {}) or {}
        last_user_message = HumanInputFormat.from_str(message_content)

        # Ali Akbar Start (Gap 1 — Hybrid Risk Estimation: re-classifies on every real user message)
        # Skip classification for any message that is NOT a genuine user task description:
        #   1. plan acceptance via Accept button  (accepted=True)
        #   2. typed acceptance strings            ("accept", "run", "looks good", …)
        #   3. messages the orchestrator tagged as internal  (metadata["internal"] == "yes")
        # Classifying these would silently reset the approval policy established by the real task.
        _is_plan_control = (
            last_user_message.accepted
            or is_accepted_str(last_user_message.content)
            or _raw_metadata.get("internal") == "yes"
        )

        # Ali Akbar Prompt Injection Start
        # Gap 2 extension: scan the user's own task text for injected instructions
        # before any agent acts on it. Independent of Gap 1's adaptive_approval — this
        # is a Gap 2 concern, gated the same way page/file/code/mcp scanning already is
        # (approval_guard present, policy != "never"), not by the Gap 1 master switch.
        if not _is_plan_control and self._approval_guard is not None:
            _task_gate = await scan_and_gate(
                last_user_message.content,
                model_client=self._model_client,
                hybrid_enabled=self._config.hybrid_injection_detection,
                action_guard=self._approval_guard,
                source_label="task",
                source_id="current task",
                blocked_ids=set(),
                context="user_task",  # BUG-02 fix: narrower prompt, see semantic_injection_detector.py
            )
            if _task_gate.blocked:
                await self._log_message_agentchat(
                    "🚨 Your task description appears to contain hidden or injected "
                    "instructions commonly used to manipulate AI agents. This task will "
                    "not be executed. Please rephrase your task and try again.",
                    metadata={"internal": "no", "type": "progress_message"},
                )
        ...
        if self._config.adaptive_approval and self._approval_guard is not None and not _is_plan_control:
            if isinstance(self._approval_guard, ApprovalGuard):
                # Respect an explicit "never" — user intentionally disabled all approvals
                if self._approval_guard.config.approval_policy != "never":

                    # ── Rule-based layer (always runs; deterministic safety floor) ──────
                    rule_task_type, rule_matched_kws = classify_task_with_keywords(
                        last_user_message.content
                    )
                    rule_risk_score = get_risk_score(rule_task_type)
                    rule_policy     = POLICY_MAP[rule_task_type]
                    rule_reason     = (
                        f"Keyword(s) matched: {', '.join(rule_matched_kws)}"
                        if rule_matched_kws
                        else "No specific keywords matched — defaulting to research"
                    )

                    # ── LLM-based layer (optional; falls back gracefully) ─────────────
                    llm_result: LLMRiskResult | None = None
                    fusion_mode = "rule-only fallback"
                    if self._config.hybrid_risk_estimation:
                        llm_result = await estimate_risk_with_llm(
                            last_user_message.content,
                            self._model_client,
                        )
                    ...
                    # ── Conservative fusion: max risk signal dominates ────────────────
                    if llm_result and llm_result.available:
                        final_risk_score = max(rule_risk_score, llm_result.risk_score)
                        # Task type: rule destructive is never downgraded
                        if rule_task_type == "destructive":
                            final_task_type = "destructive"
                        elif (
                            llm_result.task_type == "destructive"
                            and llm_result.confidence >= 0.60
                        ):
                            final_task_type = "destructive"
                        else:
                            # Derive from fused risk score
                            if final_risk_score >= 0.71:
                                final_task_type = "destructive"
                            elif final_risk_score >= 0.31:
                                final_task_type = "transactional"
                            else:
                                final_task_type = "research"
                    else:
                        final_risk_score = rule_risk_score
                        final_task_type  = rule_task_type

                    # Safety invariant: destructive must always map to max risk + "always"
                    if final_task_type == "destructive":
                        final_risk_score = max(final_risk_score, 0.71)

                    # Policy from fused risk score
                    if final_risk_score >= 0.71 or final_task_type == "destructive":
                        final_policy = "always"
                    elif final_risk_score >= 0.31:
                        final_policy = "auto-conservative"
                    else:
                        final_policy = "auto-permissive"

                    # Apply the fused policy to the approval guard
                    self._approval_guard.config.approval_policy = final_policy
                    task_type  = final_task_type
                    risk_score = final_risk_score
                    new_policy = final_policy

                    trace_logger.info(
                        f"[ADAPTIVE GUARD] rule={rule_task_type!r}/{rule_risk_score}"
                        f" llm={llm_result.task_type!r}/{llm_result.risk_score:.2f}"
                        f" (conf={llm_result.confidence:.2f})"
                        if (llm_result and llm_result.available) else
                        f"[ADAPTIVE GUARD] rule={rule_task_type!r}/{rule_risk_score}"
                        f" llm=unavailable"
                        f" final={final_task_type!r}/{final_risk_score:.2f} → policy={final_policy!r}"
                        f" fusion={fusion_mode}"
                    )

                    # All metadata values MUST be str (AutoGen TextMessage constraint)
                    _cls_meta: dict[str, str] = {
                        "internal":    "no",
                        "type":        "progress_message",
                        # Consumed by connection.py for Gap 1 badge and Gap 3 Bayesian policy
                        "_halo_classification": "true",
                        # Final (fused) values
                        "task_type":   final_task_type,
                        "policy":      final_policy,
                        "risk_score":  str(final_risk_score),
                        # Rule layer
                        "rule_task_type":       rule_task_type,
                        "rule_risk_score":      str(rule_risk_score),
                        "rule_policy":          rule_policy,
                        "rule_reason":          rule_reason,
                        "rule_matched_keywords": json.dumps(rule_matched_kws),
                        # Fusion
                        "fusion_mode": fusion_mode,
                    }
                    if llm_result is not None:
                        _cls_meta.update({
                            "llm_task_type":          llm_result.task_type,
                            "llm_risk_score":         str(llm_result.risk_score),
                            "llm_confidence":         str(llm_result.confidence),
                            "llm_reason":             llm_result.reason,
                            "llm_possible_harms":     json.dumps(llm_result.possible_harms),
                            "llm_recommended_policy": llm_result.recommended_policy,
                            "llm_available":          str(llm_result.available).lower(),
                        })

                    await self._log_message_agentchat(
                        f"[Adaptive Guard] Task classified as **{final_task_type}** "
                        f"(risk score: {final_risk_score:.2f}, fusion: {fusion_mode})"
                        f" → approval policy: **{final_policy}**",
                        metadata=_cls_meta,
                    )
        # Ali Akbar End (Gap 1)
