...
async def _invoke_action_guard(
    ...
    # Ali Akbar Prompt Injection Start — the original instruction, for hijack screening
    _instruction = str(getattr(thread[0], "content", "")) if thread else ""
    ...
    await guarded_action.invoke_with_approval(
        ...
        hybrid_injection_detection=hybrid_injection_detection,
    )
...
async def _coding_and_debug(
    ...
    Args:
        ...
        hybrid_injection_detection (bool): Whether to run the semantic/LLM injection
            layer over code-execution output (Ali Akbar Prompt Injection). Default True.
    ...
    delta: Sequence[BaseChatMessage | BaseAgentEvent] = []
    executed_code = False
    _blocked_code_ids: set[str] = set()  # Ali Akbar Prompt Injection
    ...
    for i in range(max_debug_rounds):
        ...
        try:
            for cb in code_block_list:
                ...
                # Scan code-execution output for injected instructions (e.g. a
                # downloaded file or fetched URL whose content tries to redirect the
                # coder agent), same shared gateway every other content source in
                # HALO routes through.
                if approval_guard is not None and code_output:
                    _gate = await scan_and_gate(
                        code_output,
                        model_client=model_client,
                        hybrid_enabled=hybrid_injection_detection,
                        action_guard=approval_guard,
                        source_label="code output",
                        source_id=f"round_{i + 1}",
                        blocked_ids=_blocked_code_ids,
                        block_variant="continue",
                    )
                    if _gate.blocked:
                        assert _gate.replacement_text is not None
                        code_output = _gate.replacement_text
                # Ali Akbar Prompt Injection End
...
class HALOCoderAgent(BaseChatAgent, Component[CoderAgentConfig]):
    ...
    component_provider_override = "halo.agents.HALOCoderAgent"
   ...
   VERY IMPORTANT: If you intend to write code to be executed, do not end your response without a code block. If you want to write code you must provide a code block in the current generation.
    ...
    def __init__(
        ...
        Args:
            ...
            hybrid_injection_detection (bool, optional): Whether to run the semantic/LLM
                injection layer over code-execution output and hijack-screen code
                execution requests (Ali Akbar Prompt Injection). Default: True.
        """
        ...
        self._hybrid_injection_detection = hybrid_injection_detection  # Ali Akbar Prompt Injection
    ...
    async def on_messages_stream(
        ...
        try:
            ...
            logger.error(f"Error in HALOCoderAgent: {e}")
            # add to chat history
