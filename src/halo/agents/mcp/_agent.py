...
class McpAgent(AssistantAgent):
    ...
    def configure_injection_guard(
        ...
        """Wire a live ApprovalGuard onto this agent's workbench, if it's a HALO-owned
        AggregateMcpWorkbench, so MCP tool calls/results get the same injection-scan and
        hijack-screening coverage web_surfer/file_surfer/coder already have. Called
        post-construction from task_team.py — McpAgent is built via the Component
        `_from_config` path, which only sees serializable config, not the live guard."""
        if isinstance(self._workbench, AggregateMcpWorkbench):
            self._workbench.configure_injection_guard(
                approval_guard, hybrid_injection_detection
            )
    ...
    async def on_messages_stream(
        ...
        # Ali Akbar Prompt Injection Start
        # Record what this turn's instruction is, so a subsequent call_tool() can
        # hijack-screen its proposed tool call against it.
        if messages and isinstance(self._workbench, AggregateMcpWorkbench):
            _last_content = getattr(messages[-1], "content", "")
            _instruction = (
                _last_content if isinstance(_last_content, str) else str(_last_content)
            )
            self._workbench.set_current_instruction(_instruction)
        # Ali Akbar Prompt Injection End
