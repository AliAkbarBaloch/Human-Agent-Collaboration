...
# New configuration class for HALOWebSurfer
class WebSurferConfig(BaseModel):
    ...
    # Set to False for pattern-only ablation mode.
...
class WebSurferState(BaseState):
    """
    State class for saving and loading the HALOWebSurfer's state.
...
class HALOWebSurfer(BaseChatAgent, Component[WebSurferConfig]):
    ...
    The HALOWebSurfer uses Playwright to control a browser and navigate web pages. It can:
    ...
    Args:
        ...
        description (str, optional): The description of the agent. Default: HALOWebSurfer.DEFAULT_DESCRIPTION.
        debug_dir (str, optional): The directory where debug information is saved. Default: None.
        start_page (str, optional): The start page for the browser. Defaults: HALOWebSurfer.DEFAULT_START_PAGE.
    ...
    component_provider_override = "halo.agents.web_surfer.HALOWebSurfer"
    ...
    def __init__(
        ...
        self.logger = logging.getLogger(EVENT_LOGGER_NAME + f".{self.name}.HALOWebSurfer")
        ...
        # Ali Akbar Start (Gap 2 — track user-blocked URLs to avoid re-prompting)
        # URLs the user has already chosen to block — auto-blocked without re-prompting.
    ...
    async def resume(self) -> None:
        """Resume the HALOWebSurfer agent."""
        self.is_paused = False
    ...
    async def close(self) -> None:
        ...
        """
        logger.info("Closing HALOWebSurfer...")
    ...
    async def on_reset(self, cancellation_token: CancellationToken) -> None:
        """Reset the HALOWebSurfer agent's state."""
    ...
    async def on_messages_stream(
        ...
        if self.is_paused:
            yield Response(
                chat_message=TextMessage(
                    content="The HALOWebSurfer is paused. Please resume to continue surfing the web.",
                    source=self.name,
        ...
        try:
            for _ in range(self.max_actions_per_step):
                ...
                if not need_execute_tool:
                    ...
                    for action in response:
                        ...
                        _hijack_reason: Optional[str] = None  # Ali Akbar Prompt Injection
                        ...
                        if self.use_action_guard and self.action_guard is not None:
                            ...
                            # Action-hijack screening — independent of the risk check
                            # above. Asks whether this proposed action looks like it
                            # was redirected by page content read earlier, rather than
                            # serving the instruction this turn. Forces approval if so,
                            # even under a policy/baseline that would otherwise skip it.
                            if self._hybrid_injection_detection and is_scan_active(self.action_guard):
                                _hijack = await screen_action_for_hijack(
                                    self._last_outside_message, tool_call_msg, self._model_client
                                )
                                if (
                                    _hijack.available
                            ...
                            if _hijack_reason:
                                request_message = TextMessage(
                                    ...
                                    content=(
                                        ...
                                        + f"\n\n⚠️ Possible instruction hijack detected: {_hijack_reason}"
                                    ),
                                )
                            # Ali Akbar Prompt Injection End
                        ...
                        try:
                            ...
                            if "HALOWebSurfer was paused" in str(e):
                                self.logger.info(f"Tool execution paused: {e}")
                                action_result = "Action was interrupted because HALOWebSurfer was paused."
                            else:
            ...
            yield Response(
                chat_message=TextMessage(
                    content=f"The HALOWebSurfer encountered an error: {e}",
                    source=self.name,
    ...
    async def _get_llm_response(
        ...
        tool_names = HALOWebSurfer._tools_to_names(tools)

        webpage_text = await self._playwright_controller.get_visible_text(self._page)

        # Ali Akbar Prompt Injection Start
        # Scan full page text (including below-the-fold) before the LLM sees it, via the
        # shared injection_gateway (same scan/fuse/block pipeline every other content
        # source in HALO now routes through — see injection_gateway.py).
        # Priority: document.body.innerText > get_page_markdown > viewport text.
        _page_was_blocked = False
        gate = None
        try:
            current_url = self._page.url
            if current_url in self._blocked_urls:
                # Already blocked earlier — skip the (expensive) full-page text fetch.
                gate = await scan_and_gate(
                    "",
                    ...
                    blocked_ids=self._blocked_urls,
                    block_variant="continue",
                )
            else:
                # Build full_page_text: try full-page sources before falling back to viewport.
                full_page_text = webpage_text  # fallback: viewport only

                # First try document.body.innerText — fast, always includes below-fold text.
                try:
                    all_text = await self._page.evaluate("() => document.body.innerText")
                    if all_text and len(all_text) >= len(webpage_text):
                        full_page_text = all_text
                        logger.debug(f"[InjectionScan] using innerText ({len(all_text)} chars)")
                except Exception as _e:
                    logger.debug(f"[InjectionScan] innerText failed: {_e}")

                # Also try get_page_markdown — richer, catches CSS-hidden elements.
                try:
                    page_md = await self._playwright_controller.get_page_markdown(self._page)
                    if page_md:
                        full_page_text = page_md
                        logger.debug(f"[InjectionScan] using page_markdown ({len(page_md)} chars)")
                except Exception as _e:
                    logger.debug(f"[InjectionScan] get_page_markdown failed: {_e}")
            ...
            _page_was_blocked = gate.blocked
        except Exception as _scan_exc:
            logger.warning(f"[InjectionScan] scan raised unexpectedly: {_scan_exc}")

        # If the user blocked this page, replace webpage_text with a strong directive.
        # We do NOT early-return because the LLM still needs to be able to navigate away
        # if the current task requires a different URL.
        if _page_was_blocked and gate is not None and gate.replacement_text is not None:
            logger.warning(f"[InjectionScan] page blocked — injecting blocked-page directive for {self._page.url!r}")
            webpage_text = gate.replacement_text
        # Ali Akbar Prompt Injection End
        ...
        if self.is_multimodal:
            ...
            # Ali Akbar Start (Gap 2 — omit screenshots when page is blocked; prevents visual content leak)
            if _page_was_blocked:
                # Page was blocked — send only the text directive, no screenshots.
                # Including screenshots would let the LLM read blocked page content visually.
                scaled_som_screenshot.close()
    ...
    async def _execute_tool(
        ...
        Raises:
            ...
            RuntimeError: If the HALOWebSurfer was paused during tool execution
        """
        ...
        try:
            ...
            return f"HALOWebSurfer was paused, action '{name}' was cancelled."
        finally:
    ...
    async def _summarize_page(
        ...
        # Ali Akbar Prompt Injection Start
        # Same shared injection_gateway pipeline used everywhere else in HALO now.
        if page_markdown:
            current_url = self._page.url
            gate = await scan_and_gate(
                page_markdown,
            ...
            if gate.blocked:
                # Navigate away so next run does not start on the blocked page.
                await self._page.goto("about:blank")
                assert gate.replacement_text is not None
                return gate.replacement_text
        # Ali Akbar Prompt Injection End
    ...
    def _to_config(self) -> WebSurferConfig:
        return WebSurferConfig(
            ...
            hybrid_injection_detection=self._hybrid_injection_detection,
        )
    ...
    def _from_config(cls, config: WebSurferConfig) -> Self:
        return cls(
            ...
            hybrid_injection_detection=config.hybrid_injection_detection,
        )
    ...
    async def save_state(self, save_browser: bool = True) -> Mapping[str, Any]:
        """Save the current state of the HALOWebSurfer.
        Args:
        ...
        # Create and return the HALOWebSurfer state
        state = WebSurferState(
            chat_history=self._chat_history,
