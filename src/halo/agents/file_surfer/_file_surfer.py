...
class FileSurferConfig(BaseModel):
    """Configuration for HALOFileSurfer agent
...
class FileSurferState(BaseState):
    """
    State class for saving and loading the HALOFileSurfer's state.
...
class HALOFileSurfer(BaseChatAgent, Component[FileSurferConfig]):
    """
    ...
    component_provider_override = "autogen_ext.agents.file_surfer.HALOFileSurfer"
    ...
    async def pause(self) -> None:
        """Pause the HALOFileSurfer agent."""
        self.is_paused = True
        self._pause_event.set()

    async def resume(self) -> None:
        """Resume the HALOFileSurfer agent."""
        self.is_paused = False
        self._pause_event.clear()

    @property
    def produced_message_types(self) -> Sequence[type[BaseChatMessage]]:
        """Get the types of messages produced by the HALOFileSurfer agent."""
        return (TextMessage,)
    ...
    async def on_messages_stream(
        ...
        try:
            ...
            # The system message must come first — some hosted backends (e.g. the
            # litellm/vLLM endpoint this project also targets) reject a request
            # outright with "System message must be at the beginning" otherwise,
            # which silently broke every file_surfer turn once any prior history
            # existed (the coder/web_surfer agents already put it first; this one
            # didn't).
            try:
                await self._model_context.clear()
                for msg in (
                    default_system_messages + history + [context_message, task_message]
                ):
            ...
            if isinstance(response, str):
                ...
                for function_call in function_calls:
                    ...
                    # Ali Akbar Prompt Injection Start
                    # Scan the file content this action just surfaced, same shared
                    # gateway every other content source in HALO routes through.
                    if guarded_action is not None and content_action:
                        _source_id = self._browser.path or "unknown"
                        _gate = await scan_and_gate(
                            ...
                            blocked_ids=self._blocked_paths,
                            block_variant="continue",
                        )
                        if _gate.blocked:
                            assert _gate.replacement_text is not None
                            content_action = _gate.replacement_text
                    # Ali Akbar Prompt Injection End
    ...
    async def on_reset(self, cancellation_token: CancellationToken) -> None:
        """Reset the HALOFileSurfer agent's state."""
    ...
    async def save_state(self) -> Mapping[str, Any]:
        """
        Save the current state of the HALOFileSurfer.
    ...
    async def close(self) -> None:
        """Close the HALOFileSurfer agent."""
        logger.info("Closing HALOFileSurfer...")
    ...
    def _to_config(self) -> FileSurferConfig:
        """
        Convert the HALOFileSurfer instance to a configuration object.
    ...
    def _from_config(cls, config: FileSurferConfig) -> Self:
        """
        Create a HALOFileSurfer instance from a configuration object.

        Args:
            config (FileSurferConfig): A `FileSurferConfig` object containing the configuration.

        Returns:
            HALOFileSurfer: A new instance of the HALOFileSurfer class.
        """
        code_executor = None
