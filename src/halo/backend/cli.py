...
app = typer.Typer(help="HALO: A human-centered interface for web agents.")
...
def get_env_file_path():
    ...
    """
    app_dir = os.path.join(os.path.expanduser("~"), ".halo")
...
# invoke_without_command=True means this function runs automatically when only 'halo-app' is typed
...
def main(
    ...
    version: bool = typer.Option(
        False, "--version", help="Print the version of HALO and exit."
    ),
    ...
    """
    HALO: A human-centered interface for web agents.

    Run `halo-app` to start the application.
    """
    # Check if version flag was provided
    if version:
        typer.echo(f"HALO version: {VERSION}")
        raise typer.Exit()

    # This conditional checks if a subcommand was provided
    # If no subcommand was specified (e.g., just 'halo-app'), run the UI
...
def run_ui(
    ...
    """
    Core logic to run the HALO web application.
    ...
    Args:
        ...
        appdir (str, optional): Path to the app directory where files are stored. Defaults to ~/.halo.
    ...
    """
    # Display a green, bold "Starting HALO" message
    typer.echo(typer.style("Starting HALO", fg=typer.colors.GREEN, bold=True))
    ...
    if not run_without_docker:
        ...
        typer.echo(
            typer.style(
                "For the full experience of HALO please use docker.",
                fg=typer.colors.YELLOW,
                bold=True,
            )
        )
    ...
    # When workers=1, do NOT pass the workers parameter — passing workers=1 explicitly
    # triggers uvicorn's multiprocessing supervisor (even for a single worker), which
    # uses different signal handling and causes the server to exit immediately on macOS
    # with Python 3.12. Omitting it runs the simpler single-process mode.
    uvicorn_kwargs: dict = {
        "host": host,
        "port": port,
        "reload": reload,
    ...
    uvicorn.run("halo.backend.web.app:app", **uvicorn_kwargs)
...
def version():
    """
    Print the version of the HALO backend CLI.
    """
    typer.echo(f"HALO version: {VERSION}")
...
def run():
    """
    Main entry point called by the 'halo' and 'halo-app' commands.
...
if __name__ == "__main__":
    app()  # Allow running this file directly with 'python -m halo.backend.cli'
