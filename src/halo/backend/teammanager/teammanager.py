...
from ...agents import HALOWebSurfer
...
class TeamManager:
    ...
    def __init__(
        ...
        # Ali Akbar Start (Gap 2 — persist blocked URLs across runs so re-visiting a blocked page auto-blocks)
        self.blocked_urls: set[str] = set()
        # Ali Akbar End (Gap 2)
        # Ali Akbar Start (Gap 3 — expose the resolved config and shared ApprovalGuard
        # so WebSocketManager can read enable_trust_feedback and write trust-adjusted
        # policy back into live enforcement)
        self.halo_config: HALOAppConfig | None = None
        self.approval_guard: Any = None
        # Ali Akbar End (Gap 3)
    ...
    async def _create_team(
        ...
        *,
        ...
        try:
            # Logic here: we first see if the config file passed to halo-app has valid configs for all clients
            # If Yes: this takes precedent over the UI LLM config and is passed to halo-app team
            ...
            # Logic here: first, we see if the config file passed to halo-app has valid MCP agent configuration
            ...
            # Ali Akbar Start (Gap 1 — read adaptive_approval from YAML; no UI toggle exists for this flag)
            if isinstance(self.config, dict) and "adaptive_approval" in self.config:
                config_params["adaptive_approval"] = bool(self.config["adaptive_approval"])
            # Ali Akbar End (Gap 1)
            # Ali Akbar Start (Hybrid Gap 1 & 2 — read hybrid flags from YAML for ablation support)
            if isinstance(self.config, dict) and "hybrid_risk_estimation" in self.config:
                config_params["hybrid_risk_estimation"] = bool(self.config["hybrid_risk_estimation"])
            if isinstance(self.config, dict) and "hybrid_injection_detection" in self.config:
                config_params["hybrid_injection_detection"] = bool(self.config["hybrid_injection_detection"])
            # Ali Akbar End (Hybrid)
            # Ali Akbar Start (Gap 3 — read enable_trust_feedback from YAML; no UI toggle exists for this flag)
            if isinstance(self.config, dict) and "enable_trust_feedback" in self.config:
                config_params["enable_trust_feedback"] = bool(self.config["enable_trust_feedback"])
            # Ali Akbar End (Gap 3)
            ...
            halo_config = HALOAppConfig(**config_params)  # type: ignore
            self.team = cast(
                Team,
                await get_task_team(
            ...
            # Ali Akbar Start (Gap 3 — expose resolved config + shared ApprovalGuard
            # to WebSocketManager for closed-loop trust enforcement)
            self.halo_config = halo_config
            self.approval_guard = getattr(self.team, "halo_approval_guard", None)
            # Ali Akbar End (Gap 3)
            if hasattr(self.team, "_participants"):
                for agent in cast(list[ChatAgent], self.team._participants):  # type: ignore
                    if isinstance(agent, HALOWebSurfer):
                        novnc_port = agent.novnc_port
                        playwright_port = agent.playwright_port
                        # Ali Akbar Start (Gap 2 — share session-level blocked_urls so blocks persist across runs)
                        agent._blocked_urls = self.blocked_urls
                        # Ali Akbar End (Gap 2)
