...
class Run(SQLModel, table=True):
    ...
    # Ali Akbar — HALO persistent state (all three Gap layers, serialized per-run)
    halo_state: Optional[dict[str, Any]] = Field(default=None, sa_column=Column(JSON))
...
# Ali Akbar Start (Gap 3 — Bayesian Trust Adaptation, cross-run persistence)
class TrustProfile(SQLModel, table=True):
    """Per-user Beta(alpha, beta) trust parameters, persisted across runs/sessions.

    One row per user_id. Loaded into a fresh run's BayesianFeedbackLoop as the
    starting prior (instead of the hardcoded defaults) when
    HALOAppConfig.enable_trust_feedback is set, so approve/reject/injection-block
    history compounds over time rather than resetting every task.
    """
    ...
    user_id: Optional[str] = Field(default=None, unique=True)
    alpha: dict[str, float] = Field(default_factory=dict, sa_column=Column(JSON))
    beta: dict[str, float] = Field(default_factory=dict, sa_column=Column(JSON))
...
DatabaseModel = Team | Message | Session | Run | Gallery | Settings | Plan | TrustProfile
