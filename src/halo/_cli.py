...
from .halo_config import HALOAppConfig, ModelClientConfigs, SentinelPlanConfig
...
async def cancellable_input(
    ...
    assert input_type in ["text_input", "approval", "injection_alert"]
    log_debug(
...
async def get_team(
    ...
    debug: bool = False,
    state_file: str = ".halo_state.json",
    ...
    log_debug(
        f"HALOAppConfig created with planning={cooperative_planning}, execution={autonomous_execution}",
        debug,
    )
...
def display_halo_logo():
    """Display the HALO entry text."""

    halo_logo = f"""{MAGENTA}{BOLD}╔═══════════════════════════════════════════════════════════════════╗
...
def cli_main(
    ...
    mcp_agents_file: Optional[str] = typer.Option(
        None,
        "--mcp-agents-file",
        help="Path to a .yaml file containing configuration compatible with HALOAppConfig.mcp_agents",
    ),
    ...
    """
    HALO CLI: A command-line interface for multi-agent task execution.
    ...
    def signal_handler(sig: int, frame: types.FrameType | None) -> Any:
        ...
        logger.info("halo-app cli caught SIGINT...")
...
def main() -> None:
    """
    Entry point for the halo-cli command.
