from .input_func import InputFuncType, AsyncInputFunc, SyncInputFunc, InputRequestType
...
class BaseApprovalGuard(Protocol):
    ...
    # Ali Akbar Start (Gap 2 — added input_type parameter to support injection_alert)
    async def get_approval(
        self,
        action_description: TextMessage | MultiModalMessage,
        input_type: InputRequestType = "approval",
    ) -> bool:
...
class ApprovalGuardContext:
    ...
    def approval_guard(cls) -> BaseApprovalGuard | None:
        try:
            ...
            raise RuntimeError(
                "ApprovalGuardContext.approval_guard() should only be called when initializing a HALOWebSurfer agent during Team creation."
            ) from e
...
class ApprovalGuard(BaseApprovalGuard):
    ...
    # Ali Akbar Start (Gap 2 — added input_type parameter so injection_alert is forwarded to UI)
    async def _get_input(
        ...
        try:
            if self._is_async:
                ...
                return await async_func(prompt, cancellation_token, input_type)
            else:
                # Cast to SyncInputFunc for proper typing
                sync_func = cast(SyncInputFunc, self.input_func)
                loop = asyncio.get_event_loop()
                return await loop.run_in_executor(None, sync_func, prompt, input_type)
    ...
    # Ali Akbar Start (Gap 2 — added input_type parameter so injection_alert reaches the UI)
    async def get_approval(
        self,
        action_description: TextMessage | MultiModalMessage,
        input_type: InputRequestType = "approval",
        ...
        result_or_json = await self._get_input(action_description_str, None, input_type)
