...
class HALOAppConfig(BaseModel):
    """
    A simplified set of configuration options for HALO.
    ...
    Attributes:
        ...
        enable_trust_feedback (bool): Gap 3 closed-loop switch. When True (default), the Bayesian trust
            score actually gates live approvals (in-run) and persists per-user across runs. Set False to
            fall back to display-only trust, e.g. for ablation studies. Default: True.
    """
    ...
    # Ali Akbar End (Gap 1)
    # Ali Akbar Start (Hybrid Gap 1 & 2 — LLM-based components)
    # hybrid_risk_estimation: combines rule-based + LLM risk estimator (Gap 1)
    # hybrid_injection_detection: combines pattern + semantic LLM detector (Gap 2)
    # Set either to False for ablation studies (pure rule-based / pattern-only mode).
    hybrid_risk_estimation: bool = True
    hybrid_injection_detection: bool = True
    # Ali Akbar End (Hybrid)
    # Ali Akbar Start (Gap 3 — Bayesian Trust Adaptation, closed-loop enforcement)
    # When True (default), Gap 3's trust score closes the loop:
    #   - the live ApprovalGuard.config.approval_policy is overwritten with the
    #     current Bayesian-derived policy for the task's type, both right after
    #     Gap 1 classification and after every subsequent approve/reject/injection
    #     decision in the run (so trust actually tightens/loosens enforcement).
    #   - the per-user Beta(alpha, beta) trust parameters are loaded from and
    #     persisted to the TrustProfile DB table, so trust compounds across runs
    #     instead of resetting to the default prior every task.
    #   - the destructive-task safety floor (always "always") is unaffected either
    #     way; see BayesianFeedbackLoop.get_policy().
    # Set to False for ablation studies (Gap 3 becomes display-only, as upstream).
    enable_trust_feedback: bool = True
    # Ali Akbar End (Gap 3)
